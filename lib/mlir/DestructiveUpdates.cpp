...
void insertReuse(mlir::Value reuseVal,Region &region){
  ...
  Operation *resetOp = builder.create<lambdapure::ResetOp>(builder.getUnknownLoc(),reuseVal,2);
...
void insertReuseConstructor(mlir::Region &region){
  ...
  for(auto op = region.op_begin(); op != region.op_end();++op){
    ...
    if(name == "lambdapure.ResetOp"){
      ...
      for(auto inner_op = resetRegion.op_begin(); inner_op != region.op_end();++inner_op){
        ...
        if(inner_name == "lambdapure.ConstructorOp"){
            ...
            for(auto operand = inner_op -> operand_begin(); operand != inner_op -> operand_end();++operand){
  ...
  bool checkControlPath(mlir::Value candidate,Operation *op){
    for(auto user_it = candidate.user_begin(); user_it != candidate.user_end();++user_it){
