...
namespace{
  ...
  private:
    ...
    void translateAllocCtorOp(Operation* op){
      ...
      int size = op -> getAttrOfType<IntegerAttr>("size").getInt();
      std::string varName = varTable.get(op);
      outFile << "lean_object* " << varName <<  " = " << "lean_alloc_ctor(" << tag << "," << size << ",0);\n";
    }
    ...
    void translateReturnOP(Operation* op){
      // auto parentName = op -> getParentOp() -> getAttrOfType<StringAttr>("sym_name").getValue().str();
      // std::string returnVar = "";
      // if(parentName != "_lean_main"){
    ...
    void translateProjectionOp(Operation* op){
      ...
      std ::string o1 = varTable.get(op -> getOperand(0));
      std::string varName = varTable.get(op);
      outFile << "lean_object* " << varName << " = lean_ctor_get(" << o1 << ", " << index << ");\n" ;
    }
    void translateAppOp(Operation* op){
      ...
      outFile << "lean_object* " << varName << " = " << "lean_apply_" << (n-1) << "(" << callee << ", ";
    ...
    void translateUnboxOp(Operation* op){
      std::string varName = varTable.get(op);
      std::string o1 = varTable.get(op -> getOperand(0));
      outFile << "int " << varName << " = " << "lean_unbox(" << o1 << "0;\n";
    }
