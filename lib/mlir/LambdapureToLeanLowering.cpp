...
namespace {
  ...
  public:
    ...
    ConversionPattern(lambdapure::IntegerConstOp::getOperationName(),1,ctxt){}
    ...
    (Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const final {
      ...
      rewriter.replaceOpWithNewOp<lambdapure::BoxOp>(op,objType,op -> getAttrOfType<IntegerAttr>("value"));
      return success();
    }
  };
  ...
  public:
    ...
    ConversionPattern(lambdapure::ConstructorOp::getOperationName(),1,ctxt){}
    LogicalResult matchAndRewrite
    (Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const final {
      int i = 0;
      auto loc = op->getLoc();

      mlir::Value alloc_ctor =  rewriter.create<lambdapure::AllocCtorOp>(loc,
          op->getResultTypes(),
      ...
      for(auto it = op -> operand_begin();it != op -> operand_end();++it){
        rewriter.create<lambdapure::CtorSetOp>(loc,
          rewriter.getI64IntegerAttr(i), alloc_ctor, *it);
        i++;
      }
  ...
  public:
    ...
    (Operation *op, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const final {
      ...
      int i = 0;
      for(auto it = op -> operand_begin() + 1;it != op -> operand_end();++it){ //+ 1 to remove reuseObject operand
        rewriter.create<lambdapure::CtorSetOp>(loc,
          rewriter.getI64IntegerAttr(i), alloc_ctor, *it);
        i++;
      }
...
namespace {
  struct LambdapureToLeanLoweringPass
          : public PassWrapper<LambdapureToLeanLoweringPass, OperationPass<ModuleOp>>{
...
void LambdapureToLeanLoweringPass::runOnOperation(){
  ...
  target.addIllegalOp<lambdapure::IntegerConstOp,lambdapure::ConstructorOp,lambdapure::ReuseConstructorOp>();
